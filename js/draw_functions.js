...
function addEffectToSprite(ctx, script, sprite){
	var temporaryObj = {};
	var imgBox = getBox(sprite.area_2); 	
	var cutWidth = imgBox[1][0] - imgBox[0][0]; var cutHeight = imgBox[1][1] - imgBox[0][1];
	var cutArea = getCutSize(sprite.area_2, imgBox[0][0], imgBox[0][1]); 
	var cutPathArea =  getPathArea(cutArea); 
	ctx.putImageData(saveImg, 0, 0);
	var H , W;	
	H = cutHeight; W = cutWidth;
	...
	//canvas2.setAttribute("style", "width:"+sprite.frame.width+"px; height: "+sprite.frame.height+"px;");
	canvas2.width = sprite.frame.width;
    canvas2.height = 	sprite.frame.height;
	ctx2.drawImage(sprite.frame, 0, 0, sprite.frame.width, sprite.frame.height);
	var imgMap2 = ctx2.getImageData(0, 0, sprite.frame.width , sprite.frame.height);
			for(var tmpX = 0; tmpX <  W; tmpX++) {
				for(var tmpY = 0;  tmpY < H; tmpY++) {
									
					var point = (tmpY*W+tmpX)*4; 								
					if(ctx.isPointInPath(cutPathArea, tmpX, tmpY)){											
						var arr_ = runEval_(imgMap.data[ point], imgMap.data[ point+1], imgMap.data[ point+2], imgMap.data[ point+3],
						imgMap2.data[point], imgMap2.data[point+1], imgMap2.data[point+2], imgMap2.data[point+3]);
						imgMap.data[ point] = arr_ [0]; imgMap.data[ point+1] = arr_ [1]; imgMap.data[ point+2] = arr_ [2]; imgMap.data[ point+3] = arr_ [3];
						//if( tmpX ==10 )console.log(arr_[0], arr_[1], imgMap2.data[point], imgMap2.data[point+1], imgMap2.data[point+2], imgMap2.data[point+3]);	
					}								
				}
			}
			ctx.putImageData(imgMap, imgBox[0][0], imgBox[0][1]);
		function runEval_(R,G,B,A, R1,G1,B1,A1){	
			eval(script);	
			return [R,G,B,A];
		}
}
