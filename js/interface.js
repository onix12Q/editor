...
var StateMap = {
	...
	form_effects: { //цветовые эффекты
		...
		props: [ ["function_to_pixels", "inputvalue", "[name='function_to_pixels']"], ["function_to_pixels_click", "click", "[name='function_to_pixels_click']"],				  
				 ...
				 ["form_effects_class_1", 'class', "[name='form_effects_class_1']"], ["form_effects_class_2", 'class', "[name='form_effects_class_2']"], ["form_effects_class_3", 'class', "[name='form_effects_class_3']"], ["add_rm_classes_on_change_operationWith", 'emiter-operation-with', ""],
        ...
        methods: {
			add_rm_classes_on_change_operationWith: function(){ //скрывает форму при смене типа операции
				...
				if(this.emiter.prop == "common"){
					props.form_effects_class_1.removeProp("d-none");
					props.form_effects_class_2.removeProp("d-none");
					props.form_effects_class_3.removeProp("d-none");
				}else{
					props.form_effects_class_1.setProp("d-none");
					props.form_effects_class_2.setProp("d-none");
					props.form_effects_class_3.setProp("d-none");
				}				
			},			
	...
	main_form: { //основная форма
		...
		props: [ 
				 ...
				 ["cut_fone_y", 'inputvalue', "[name='cut_fone_y']"], ["cut_fone_x", 'inputvalue', "[name='cut_fone_x']"], ["fone_cut_btn", 'click', "[name='fone_cut_btn']"],  //["fone_cut_btn_back", 'click', "[name='fone_cut_btn_back']"],
		...
		methods: {
			fone_cut_btn: function(){//обрезает фоновое изображение до заданного размера
				
				var x = parseInt(this.parent.props.cut_fone_x.getProp());
				var y = parseInt(this.parent.props.cut_fone_y.getProp());
					canvas.setAttribute("style", "width:"+x+"px; height: "+y+"px;");
                    canvas.width=x;
                    canvas.height=y;
				this.$methods().renderAll();
			},
			...
			add_rm_classes_on_change_operationWith: function(){ //скрывает кнопки при операции со спрайтами и фоновой картинкой
				...
				if(this.emiter.prop == "common"){ //видимые кнопки при операциях с фоном
					...
					if(this.$props().sprites[this.$props("operationWith")].border != false){
						props.border_color.setProp(this.$props().sprites[this.$props("operationWith")].border.color);
						props.border_size.setProp(this.$props().sprites[this.$props("operationWith")].border.size);
					}					
	...
	sprites: { //спрайты
		...
		arrayProps: [
			...
			["mousewheel_scale", "emiter-mousewheel", ""],
			//["emiter_operation_with", "emiter-operation-with", ""],
		...
		arrayMethods: {
			...
			mousewheel_scale: function(){//масштабирование спрайтов колесиком мыши при наведении на нем курсора.
			   if( this.$props().sprites[this.$props("operationWith")] ){
					var sprite =this.$props().sprites[this.$props("operationWith")];
					if(!sprite.cursorOver && !sprite.stamp_cursor)return;
					var delta =  parseInt(this.emiter.prop.wheelDelta);
					if(delta > 0){						
						sprite.scale(sprite.scale_x +0.05, sprite.scale_y +0.05);
						this.$methods().renderAll();
	...
	eventEmiters: {//глобальные события для всего приложения		
		...
		["emiter-mousedown-canvas"] : {prop:  ["x", "y"]}, //событие нажатия кнопки мыщи по канвас
		["emiter-mouseup-canvas"] : {prop:  ["x", "y"]},  //события поднятия кнопки мыши
		["emiter-mousewheel"] : {prop:  ""}, //событие прокрутки колесика мыши
		["emiter-keypress"] : {prop:  null,
			behavior: function(){			
				if(this.prop["code"] == "KeyQ" && this.$props().sprites[this.$props("operationWith")]){
                    if(this.prop["rgbaArr"] == undefined){
						var props = this.$("form_effects").props;					
						var R = props.color_r.getProp(); var G = props.color_g.getProp(); var B = props.color_b.getProp();var A = props.color_a.getProp();
						this.prop.rgbaArr = [R,G,B,A];
                    }					
				}else if(this.prop["code"] == "KeyW" && this.$props().sprites[this.$props("operationWith")]){
					var props = this.$("form_effects").props;
					var R = props.color_r.getProp(); var G = props.color_g.getProp(); var B = props.color_b.getProp();var A = props.color_a.getProp();
					saveStep(saveImg, this.$props().commonProps.area_1);
					ctx.putImageData(saveImg, 0, 0);
					addEffect(ctx, this.$props().sprites[this.$props("operationWith")].area_2, [R,G,B,A]);
					saveImg = ctx.getImageData(0, 0, srcWidth , srcHeight);
					//ctx.putImageData(saveImg, 0, 0);
					this.$methods().renderAll();
					//this.$props().sprites[this.$props("operationWith")].render();
				}else if(this.prop["code"] == "KeyE" && this.$props().sprites[this.$props("operationWith")]){
					var script = this.$("form_effects").props.function_to_pixels.getProp();			
				    var area_1 = this.$props().sprites[this.$props("operationWith")].area_2;	
					saveStep(saveImg, this.$props().commonProps.area_1);
					ctx.putImageData(saveImg, 0, 0);
				    addEffectEval(ctx, area_1, script);
			        saveImg = ctx.getImageData(0, 0, srcWidth , srcHeight);
					ctx.putImageData(saveImg, 0, 0);					
			        this.$methods().renderAll();	
				}else if(this.prop["code"] == "KeyS" && this.$props().sprites[this.$props("operationWith")]){
					var script = this.$("form_effects").props.function_to_pixels.getProp();			
					saveStep(saveImg, this.$props().commonProps.area_1);
					ctx.putImageData(saveImg, 0, 0);
				    addEffectToSprite(ctx, script, this.$props().sprites[this.$props("operationWith")]);
			        saveImg = ctx.getImageData(0, 0, srcWidth , srcHeight);
					//ctx.putImageData(saveImg, 0, 0);					
			        this.$methods().renderAll();	
				}
		...
		["emiter-keyup"] : { prop:  "",
				behavior: function(){ 
					this.$$("emiter-keypress").prop = null;
				}
		}, //событие отмены нажатия кнопки клавиатуры
  ...
  document.addEventListener('mousewheel', e => {
	...
	HM.$$("emiter-mousewheel").set(event);
  }, { passive: false });
   document.addEventListener('keypress', e => {
	 HM.$$("emiter-keypress").set(event);
  });
   document.addEventListener('keyup', e => {
	 HM.$$("emiter-keyup").set(event);
  });
